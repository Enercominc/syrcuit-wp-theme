...
function syrcuit_admin_scripts() {
    ...
    wp_enqueue_style( 'syrcuit', get_template_directory_uri() . '/assets/css/admin-styles.css', '', '2022.3.13' );
}

add_action( 'admin_enqueue_scripts', 'syrcuit_admin_scripts' );


function syrcuit_admin_footer_function() {
	echo '
		<script>
		  jQuery( function() {
		    jQuery( "#syrcuit-tags-dialog" ).dialog({
		      autoOpen: false,
		      width: 600,
		    });
		 
		    jQuery( ".syrcuit-tags-opener" ).on( "click", function() {
		      jQuery( "#syrcuit-tags-dialog" ).dialog( "open" );
		    });
		  });
		</script>

		<div id="syrcuit-tags-dialog" title="About Tags">
...
add_action( 'admin_footer', 'syrcuit_admin_footer_function', 100 );
...
add_action( 'admin_head', 'syrcuit_remove_add_new_nf_button' );

function syrcuit_remove_add_new_nf_button() {
	echo '<style>
...
function syrcuit_lower_wpseo_priority( $html ) {
	return 'low';
}
add_filter( 'wpseo_metabox_prio', 'syrcuit_lower_wpseo_priority' );
